...
module.exports = class Context {
  constructor({ command = '', rootDir = process.cwd(), args = {} }) {
    ...
    // get user config form jy.config.js
  ...
  getUserConfig() {
    ...
    let xcConfigPath = '';
    if (config) {
      xcConfigPath = path.isAbsolute(config) ? config : path.resolve(this.rootDir, config);
    } else {
      xcConfigPath = path.resolve(this.rootDir, 'jy.config.js');
    }
    let userConfig = {};
    if (fse.existsSync(xcConfigPath)) {
      try {
        // eslint-disable-next-line import/no-dynamic-require
        userConfig = require(xcConfigPath);
      } catch (err) {
        log.error(`Fail to load config file ${xcConfigPath}, use default config instead`);
        console.error(err);
        process.exit(1);
      }
    } else if (this.pkg.buildConfig) {
      log.warn('You should migrate config into jy.config.js');
    }
